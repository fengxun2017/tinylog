 ...
 */
LogFile::LogFile(std::string file_name, uint64_t roll_cycle_minutes,
...
void LogFile::roll_log_file(void) {
    ...
    if (nullptr != _log_file)
    {
      ...
      std::strftime(new_file_name, sizeof(new_file_name),
                    file_name_format.c_str(), &tm_data);
      _log_file->flush();
      _log_file->close();
...
void LogFile::write_logdata(const char *logdata, uint32_t size,
    ...
    if (nullptr != _log_file)
    {
      ...
      if ((0 != _roll_cycle_minutes) &&
          ((now_minute - create_minute) >= _roll_cycle_minutes)) {
          need_roll = true;
      }
