...
BaseFile::BaseFile(const std::string file_name) {
    ...
    if (NULL == _file) {
        std::cerr
            << "[BaseFile::BaseFile] failed in logging::BaseFile::BaseFile(), fopen return NULL\n";
    } else {
        int ret = setvbuf(_file, _local_buffer, _IOFBF, sizeof(_local_buffer));
        if (0 != ret) {
            std::cerr << "[BaseFile::BaseFile] failed in logging::BaseFile::BaseFile(), setvbuf "
                         "error info:"
...
void BaseFile::append_data(const char *data, size_t size, bool flush_now) {
    ...
    if (NULL != _file)
    {
      do {
          ...
          if (n != remainder) {
              if (ferror(_file)) {
                  std::cerr << "[BaseFile::append_data] failed in logging::BaseFile::append_data, error info:"
                            << error_to_str(errno) << std::endl;
                  clearerr(_file);
                  break;
              }
    ...
    {
        std::cerr << "[BaseFile::append_data] failed in logging::BaseFile::append_data, file is NULL."<< std::endl;
    }
}
