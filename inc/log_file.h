...
* Whenever the file existence time reaches the set rolling cycle time, or the file size reaches the set rolling size, a new file will be automatically generated.
*/
class LogFile {
   public:
    /**
 ...
 * @param[in] roll_cycle_minutes File rolling period, in minutes. If the value is 0, no new files will be generated based on time rolling.
 * @param[in] roll_size_bytes File rolling size, in bytes. If the value is 0, new log files will not be rolled based on the log file size.
 */
    LogFile(std::string file_name, uint64_t roll_cycle_minutes = 0,
...
*/
    void write_logdata(const char *logdata, uint32_t size,
...
*/
   ...
   private:
    /**
* @brief The current log file is saved in the format of logfile.YMDH. A new log file is also generated.
*/
    ...
    /* When the amount of data saved in the current log file is greater than this value, a new log file is generated. */
